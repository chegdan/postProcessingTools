...
void Foam::calc(const argList& args, const Time& runTime, const fvMesh& mesh)
{
	...
	label nFaces = mesh.boundaryMesh()[patchi].size();
        if(!isA<emptyPolyPatch>(mesh.boundaryMesh()[patchi])){
			...
			dataFile<<" "<<faceCenters.boundaryField()[patchi][facei].x()<<"\t "<<faceCenters.boundaryField()[patchi][facei].y()<<"\t "<<faceCenters.boundaryField()[patchi][facei].z()<<"\t "<<nut.boundaryField()[patchi][facei]<<"\t 1\n";
			boundaryFaceCount++;
			}
		}	
	}
	...
	dataFile<<" "<<centers[cellI].x()<<"\t "<<centers[cellI].y()<<"\t "<<centers[cellI].z()<<"\t "<<nut[cellI]<<"\t 0\n";
